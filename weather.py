    ...
    def at_time(self, d_time):
        ...
        for i in range(len(self.data) - 1):
            ...
            if day_data[0] < d_time < next_day_data[0]:
                # Calculate temperature linearly between to two points
                temp_a = day_data[1]
                temp_b = next_day_data[1]
                temp_delta = temp_b - temp_a
                a_seconds = (day_data[0] - d_time).total_seconds()
                b_seconds = (d_time - next_day_data[0]).total_seconds()

                return temp_a + (temp_delta * (a_seconds / (a_seconds + b_seconds)))
...
def from_metservice(town='hamilton'):
    ...
    # Full url for the 10 day forecast from metservice
